(function(){
  ...
  _f('EXTENSION_NAME', 'Empire Lovable Vip Extension');
  _f('EXTENSION_VERSION', '1.1.7');
  _f('DEFAULT_LICENSE_USER_NAME', 'Empire Lovable Vip Extension User');
...
function extensionFooterBadge() {
  var name = typeof EXTENSION_NAME !== "undefined" ? String(EXTENSION_NAME) : "Empire Lovable Vip Extension";
...
function normalizeLicenseUserName(name) {
  ...
  if (!n || n.toLowerCase() === "test" || n.toLowerCase() === "user" || /gringow|powerkits/i.test(n)) {
    return typeof DEFAULT_LICENSE_USER_NAME !== 'undefined' ? DEFAULT_LICENSE_USER_NAME : "Empire Lovable Vip Extension User";
  }
  return n;
}
